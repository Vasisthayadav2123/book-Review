...
// node_modules/@multiavatar/multiavatar/dist/commonjs/index.js
var require_commonjs = __commonJS({
  "node_modules/@multiavatar/multiavatar/dist/commonjs/index.js"(exports, module) {
    function multiavatar(string, sansEnv, ver) {
      ...
      var CryptoJS = CryptoJS || function(h, s) {
        var f = {}, t = f.lib = {}, g = function() {
          ...
          var c = this._data, d = c.words, b = c.sigBytes, e = this.blockSize, f2 = b / (4 * e), f2;
          if (a = (f2 = a ? h.ceil(f2) : h.max((0 | f2) - this._minBufferSize, 0)) * e, b = h.min(4 * a, b), a) {
      ...
      !function(h) {
        for (var s = CryptoJS, f, t = (f = s.lib).WordArray, g = f.Hasher, f = s.algo, j = [], q = [], v = function(a2) {
          return 4294967296 * (a2 - (0 | a2)) | 0;
        }, u = 2, k = 0; 64 > k; ) {
          var l;
          a: {
            l = u;
        ...
        var a = [], f = f.SHA256 = g.extend({ _doReset: function() {
          ...
          for (var b = this._hash.words, e = b[0], f2 = b[1], m = b[2], h2 = b[3], p2 = b[4], j2 = b[5], k2 = b[6], l2 = b[7], n = 0; 64 > n; n++) {
            if (16 > n) a[n] = 0 | c[d + n];
            else {
              var r = a[n - 15], g2 = a[n - 2];
              a[n] = ((r << 25 | r >>> 7) ^ (r << 14 | r >>> 18) ^ r >>> 3) + a[n - 7] + ((g2 << 15 | g2 >>> 17) ^ (g2 << 13 | g2 >>> 19) ^ g2 >>> 10) + a[n - 16];
            }
            r = l2 + ((p2 << 26 | p2 >>> 6) ^ (p2 << 21 | p2 >>> 11) ^ (p2 << 7 | p2 >>> 25)) + (p2 & j2 ^ ~p2 & k2) + q[n] + a[n], g2 = ((e << 30 | e >>> 2) ^ (e << 19 | e >>> 13) ^ (e << 10 | e >>> 22)) + (e & f2 ^ e & m ^ f2 & m), l2 = k2, k2 = j2, j2 = p2, p2 = h2 + r | 0, h2 = m, m = f2, f2 = e, e = r + g2 | 0;
          }
          b[0] = b[0] + e | 0, b[1] = b[1] + f2 | 0, b[2] = b[2] + m | 0, b[3] = b[3] + h2 | 0, b[4] = b[4] + p2 | 0, b[5] = b[5] + j2 | 0, b[6] = b[6] + k2 | 0, b[7] = b[7] + l2 | 0;
        }, _doFinalize: function() {
          var a2 = this._data, d = a2.words, b = 8 * this._nDataBytes, e = 8 * a2.sigBytes;
          return d[e >>> 5] |= 128 << 24 - e % 32, d[14 + (e + 64 >>> 9 << 4)] = h.floor(b / 4294967296), d[15 + (e + 64 >>> 9 << 4)] = b, a2.sigBytes = 4 * d.length, this._process(), this._hash;
        }, clone: function() {
          var a2 = g.clone.call(this);
          return a2._hash = this._hash.clone(), a2;
        } });
        s.SHA256 = g._createHelper(f), s.HmacSHA256 = g._createHmacHelper(f);
      }(Math);
      var hash = "", sha256Hash, sha256Numbers;
      if (0 == string.length) return hash;
      hash = CryptoJS.SHA256(string).toString().replace(/\D/g, "").substring(0, 12);
      var p = [];
      for (var part in p.env = hash[0] + "" + hash[1], p.env = Math.round(0.47 * p.env) + "", p.clo = hash[2] + "" + hash[3], p.clo = Math.round(0.47 * p.clo) + "", p.head = hash[4] + "" + hash[5], p.head = Math.round(0.47 * p.head) + "", p.mouth = hash[6] + "" + hash[7], p.mouth = Math.round(0.47 * p.mouth) + "", p.eyes = hash[8] + "" + hash[9], p.eyes = Math.round(0.47 * p.eyes) + "", p.top = hash[10] + "" + hash[11], p.top = Math.round(0.47 * p.top) + "", p) {
        var nr = p[part];
        nr > 31 ? (1 == (nr = nr - 32 + "").length && (nr = "0" + nr), p[part] = nr + "C") : nr > 15 ? (1 == (nr = nr - 16 + "").length && (nr = "0" + nr), p[part] = nr + "B") : p[part] = 1 == (nr + "").length ? "0" + nr + "A" : nr + "A";
      }
      var final = [];
      for (var part in p) {
        var partV = p[part].substring(0, 2), theme = p[part].substring(2, 3);
      ...
      function getFinal(part2, partV2, theme2) {
        ...
        if (null != result) for (var i = 0; i < result.length; i++) resultFinal = resultFinal.replace(result[i], colors[i] + ";");
        return resultFinal;
      }
...
//# sourceMappingURL=@multiavatar_multiavatar.js.map
